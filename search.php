...
<?php
  ...
  try{
    $c=$conn->query("SELECT c_id FROM category where category like '%".$_SESSION['category']."%'");
    $c_id = $c->fetch_assoc();
    if($c_id==null)
    {
      header("location:stock_error.php");
    }
    $p=$conn->query("SELECT * from product where c_id=".$c_id['c_id']);
  }
  catch(Exception $e) {
    echo "hi";
  }
